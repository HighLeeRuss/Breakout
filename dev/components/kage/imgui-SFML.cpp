...
void Update(const sf::Vector2i& mousePos, const sf::Vector2f& displaySize,
    ...
    if (s_windowHasFocus) {
        if (io.WantSetMousePos) {
            sf::Vector2i mousePos_(static_cast<int>(io.MousePos.x),
                                  static_cast<int>(io.MousePos.y));
            sf::Mouse::setPosition(mousePos_);
        } else {
...
void Image(const sf::RenderTexture& rt,
    const sf::Color& tintColor, const sf::Color& borderColor) {
    const sf::Texture* texturePtr = &rt.getTexture();
    // sprite without texture cannot be drawn
    if (!texturePtr) {
        return;
    }
    ImTextureID textureID =
        convertGLTextureHandleToImTextureID(texturePtr->getNativeHandle());
    ImGui::Image(textureID, ImVec2((float)rt.getSize().x, (float)rt.getSize().y), ImVec2(0, 1), ImVec2(1, 0), tintColor,
        borderColor);
}

void Image(const kage::Atlas& atlas,
    const sf::Color& tintColor, const sf::Color& borderColor) {
    const sf::Texture* texturePtr = atlas.getSprite()->getTexture();
    // sprite without texture cannot be drawn
    if (!texturePtr) {
        return;
    }
    ImTextureID textureID =
        convertGLTextureHandleToImTextureID(texturePtr->getNativeHandle());
    ImGui::Image(textureID, ImVec2((float)texturePtr->getSize().x, (float)texturePtr->getSize().y), ImVec2(0, 0), ImVec2(1, 1), tintColor,
        borderColor);
}
...
bool ImageButton(const kage::Atlas& atlas, 
    const int framePadding, const sf::Color& bgColor,
    const sf::Color& tintColor) {
    const sf::Texture* texturePtr = atlas.getSprite()->getTexture();
    if (!texturePtr) {
    ...
    sf::FloatRect spriteSize = atlas.getSprite()->getGlobalBounds();
    return ::imageButtonImpl(
        *texturePtr, static_cast<sf::FloatRect>(atlas.getSprite()->getTextureRect()), sf::Vector2f(spriteSize.width, spriteSize.height),
        framePadding, bgColor, tintColor);
}
